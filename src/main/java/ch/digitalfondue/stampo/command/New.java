...
public class New implements Opts {
  ...
  private final List<String> VALID_ARCHETYPES = Arrays.asList(ARCHETYPE_BASIC, ARCHETYPE_SITE,
      ARCHETYPE_BLOG, ARCHETYPE_DOC);

  public New() {
    archetypeParam =
        optionParser.accepts("archetype").withRequiredArg().ofType(String.class)
            .defaultsTo(ARCHETYPE_BASIC);
  ...
  private Path inputPath() {
    ...
    return Paths.get(path.orElse("./" + n)).toAbsolutePath().normalize();
  }
  ...
  public void run() {
    ...
    System.out.println("Destination folder: " + outputPath);

    // extract resources in the path
    try (ZipInputStream stream =
        new ZipInputStream(Thread.currentThread().getContextClassLoader()
            .getResourceAsStream("templates/" + archetype + ".zip"))) {
      ZipEntry ze;
      while((ze = stream.getNextEntry()) != null) {
        if(!ze.isDirectory()) {
          extractFile(outputPath, ze, stream);
        }
      }
    } catch (IOException e) {
    ...
    System.out.println("Successfully create template " + archetype + " in " + outputPath);
  }
